import { kolomPegawai, dataPegawai } from '@/constant/data';
...
export default function PegawaiPage() {
  ...
  const pages = Math.ceil(dataPegawai.length / rowsPerPage);

  const items = useMemo(() => {
    const start = (page - 1) * rowsPerPage;
    const end = start + rowsPerPage;

    return dataPegawai.slice(start, end);
  }, [page]);
    ...
    <section className="min-h-screen bg-gray-100">
      ...
      <div className="p-4">
        <Table
          ...
          <TableHeader columns={kolomPegawai}>
            {(pegawai) => (
              <TableColumn key={pegawai.key}>{pegawai.label}</TableColumn>
            )}
          </TableHeader>
